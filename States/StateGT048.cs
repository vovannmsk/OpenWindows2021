...
namespace States
{
    public class StateGT048 : IState
    {
...
//            return botwindow.isOpenMenuPet();     //сделать проверку, открыто ли окно с петом Alt+P
        ...
        {
            return new StateGT049(botwindow);
        }
        ...
        {
            if (!botwindow.isHwnd()) return new StateGT057(botwindow);  //последнее состояние движка, чтобы движок сразу тормознулся
            if (server.isLogout())
            {
