...
namespace States
{
    public class StateGT044 : IState
    {
        ...
        public StateGT044(botWindow botwindow)   //, GotoTrade gototrade)
        {
        ...
        {
            return new StateGT045(botwindow);
        }
        ...
        {
            if (!server.isHwnd()) return new StateGT050(botwindow);  //последнее состояние движка, чтобы движок сразу тормознулся
            if (server.isLogout())
            {
                return new StateGT043(botwindow);  //коннект и далее
            }
            else
            {
                return this;
